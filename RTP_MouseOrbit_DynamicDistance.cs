	...
	void LateUpdate ()
	{
		...
		if (target && targetFocus) {
			...
			if (!lastLMBState && Input.GetMouseButton (0)) {
				...
				if (surfaceColliders != null && Camera.main != null) { // tutaj mała zmiana, żeby wykrywało kamerę.
			...
			if (DraggingObject) {
				if (Input.GetMouseButton (0) && !disableSteering) {
					req_ObjxSpeed += (Input.GetAxis ("Mouse X") * xObjSpeed * 0.02f - req_ObjxSpeed) * Time.unscaledDeltaTime * 10;
					req_ObjySpeed += (Input.GetAxis ("Mouse Y") * yObjSpeed * 0.02f - req_ObjySpeed) * Time.unscaledDeltaTime * 10;
				} else {
					req_ObjxSpeed += (0 - req_ObjxSpeed) * Time.unscaledDeltaTime * 4;
					req_ObjySpeed += (0 - req_ObjySpeed) * Time.unscaledDeltaTime * 4;
				}

				req_xSpeed += (0 - req_xSpeed) * Time.unscaledDeltaTime * 4;
				req_ySpeed += (0 - req_ySpeed) * Time.unscaledDeltaTime * 4;
			} else {
				if (Input.GetMouseButton (0) && !disableSteering) {
					req_xSpeed += (Input.GetAxis ("Mouse X") * xSpeed * 0.02f - req_xSpeed) * Time.unscaledDeltaTime * 10;
					req_ySpeed += (Input.GetAxis ("Mouse Y") * ySpeed * 0.02f - req_ySpeed) * Time.unscaledDeltaTime * 10;
				} else {
					req_xSpeed += (0 - req_xSpeed) * Time.unscaledDeltaTime * 4;
					req_ySpeed += (0 - req_ySpeed) * Time.unscaledDeltaTime * 4;
				}
			...
			cur_ObjxSpeed += (req_ObjxSpeed - cur_ObjxSpeed) * Time.unscaledDeltaTime * 20;
			cur_ObjySpeed += (req_ObjySpeed - cur_ObjySpeed) * Time.unscaledDeltaTime * 20;
			...
			cur_xSpeed += (req_xSpeed - cur_xSpeed) * Time.unscaledDeltaTime * 20;
			cur_ySpeed += (req_ySpeed - cur_ySpeed) * Time.unscaledDeltaTime * 20;
			x += cur_xSpeed;
			y -= cur_ySpeed;
			...
			if (surfaceColliders != null) {
				...
				if (surfaceFound) {
					cur_distance += (reqDistance - cur_distance) * Time.unscaledDeltaTime * 4;
			...
			Vector3 position = rotation * new Vector3 (0.0f, 0.0f, -distance) + targetFocus.position; // TUTAJ NASTĄPIŁA ZMIANA!
